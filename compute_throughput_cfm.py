"""
Compute throughput using the CFM-based NLI model (updated to match cfm_network.ipynb).

Multi-band (O+E+S+C+L) with per-band noise figure, transceiver SNR, and
occupancy-aware per-link NLI via ISRS ODE power evolution.

Reads RWA results from MongoDB, computes NLI using closed-form GN integrals
with full ISRS ODE power evolution.  Results are written back to MongoDB.
