...
if __name__ == "__main__":
    ...
    BAND_SELECTION = "C"  # Keep C-band fixed for single-fibre low-traffic runs
    ...
    def make_masked_uniform_tc(graph, zero_ratio=0.1, seed=42, normalize=False, active_pair_weight=1.0):
        """Uniform all-pairs traffic with a random symmetric mask that zeros out some node pairs."""
        ...
        keep_mask = rng.random(len(upper_idx[0])) >= zero_ratio

        T[:, :] = 0.0
        T[upper_idx] = keep_mask.astype(float) * active_pair_weight
        T[(upper_idx[1], upper_idx[0])] = keep_mask.astype(float) * active_pair_weight
    ...
    mask_zero_ratio = 0.21  # e.g. 0.8 means 80% of node pairs are forced to zero
    # cost266 = 0.09
    mask_seed = 42
    normalize_tc = False
    active_pair_weight = 1.0
    print(f"Fiber Span Length: {span_length_km} km")
    print(f"Traffic Mask Zero Ratio: {mask_zero_ratio:.0%}")
    print(f"Normalize Traffic Matrix: {normalize_tc}")
    print(f"Active Pair Weight: {active_pair_weight}")
    print("=" * 60)
    collection = "real"
    ...
    graph_list_raw = nt.Database.read_topology_dataset_list(db, collection, find_dic={"name": "germany50"},
                                                            node_data=True)
    ...
    graph_list = [(graph, _id, make_masked_uniform_tc(
        graph,
        zero_ratio=mask_zero_ratio,
        seed=mask_seed + idx,
        normalize=normalize_tc,
        active_pair_weight=active_pair_weight,
    ))
                  for idx, (graph, _id) in enumerate(graph_list_raw)]
