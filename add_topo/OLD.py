...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=["off", "full", "chunk"],
        help="FWM calculation mode (off | full | chunk)",
    )
    parser.add_argument(
        "--fwm_chunk_size",
        type=int,
        default=None,
        help="Chunk size for FWM in chunk mode",
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    # Physical parameters
    SPAN_LENGTH_KM = 64          # Fiber span length [km]80/64/48/32/16
    ...
    FWM_CHUNK_SIZE = 8           # Used only when FWM_MODE == "chunk"
    ...
    fwm_mode   = cli.fwm_mode        or FWM_MODE
    fwm_chunk  = cli.fwm_chunk_size  if cli.fwm_chunk_size is not None else FWM_CHUNK_SIZE
    ...
    print(f"FWM mode         : {fwm_mode}")
    if fwm_mode == "chunk":
        print(f"FWM chunk size   : {fwm_chunk}")
